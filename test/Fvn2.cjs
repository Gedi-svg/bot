...
const chainlinkFeeds = {
    ...
    "0xB0897686c545045aFc77CF20eC7A532E3120e0F1": "0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0"  // SHIB/USD
};
...
const tokenDetails = {
    ...
    "0xB0897686c545045aFc77CF20eC7A532E3120e0F1": { decimals: 18, symbol: "SHIB" }
};

const MAX_PATH_LENGTH = 6; // Configurable maximum path length
...
async function constructPaths(tokens, factoryContract) {
    ...
    logWithTimestamp('info', 'Graph constructed', { edgeCount: graph.length });
    ...
    function bellmanFord(tokens, startToken) {
        ...
        for (const [from, to, weight] of graph) {
            if (distances[from] !== Infinity && distances[from] + weight < distances[to]) {
                logWithTimestamp('info', 'Negative cycle detected', { from, to, weight });
                let curr = to;
                const path = [];
                ...
                while (!visited.has(curr) && steps < tokens.length * 2) {
                    ...
                    if (!curr) {
                        logWithTimestamp('warn', 'No predecessor found, breaking cycle trace', { curr });
                        break;
                    }
                    steps++;
                ...
                if (curr && visited.has(curr)) {
                    ...
                    cycle.push(curr);
                    if (cycle.length >= 3 && cycle.length <= MAX_PATH_LENGTH && cycle[0] === cycle[cycle.length - 1]) {
                        paths.push(cycle);
                        logWithTimestamp('info', 'Valid cycle found', { cycle: cycle.join(' -> '), length: cycle.length });
                    } else {
                    ...
                    logWithTimestamp('warn', 'No valid cycle found in trace', { path: path.join(' -> ') });
                }
            }
        }
